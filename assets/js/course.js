(() => {
  ...
  shell?.addEventListener("click", (event) => {
    if (shell.classList.contains("sidebar-open") && event.target === shell) {
      closeSidebar();
    }
  });
  ...
  const sections = [
    ...document.querySelectorAll(".lesson-article > section[id]"),
  ];
  ...
  const rememberMermaidSources = () => {
    document.querySelectorAll(".mermaid").forEach((element) => {
      if (element.dataset.devopsMermaidSource) {
        return;
      }
      ...
      if (!element.querySelector("svg")) {
        ...
        if (source) {
          element.dataset.devopsMermaidSource = source;
        }
      }
    });
  };
  ...
  const renderMermaid = (theme) => {
    mermaidRenderSequence = mermaidRenderSequence.then(async () => {
      ...
      const nodes = [...document.querySelectorAll(".mermaid")].filter(
        (element) => Boolean(element.dataset.devopsMermaidSource),
      );
      ...
      try {
        ...
        window.dispatchEvent(
          new CustomEvent("devops-mermaid-rendered", {
            detail: { theme },
          }),
        );
      } catch (error) {
...
(() => {
   ...
   */
  const positionActiveTocItem = (smooth = true) => {
    const links = [...toc.querySelectorAll("a")];

    const active = toc.querySelector("a.is-active");

    if (!active || !links.length || active === lastActive) {
      return;
    }
    ...
    frameId = requestAnimationFrame(() => {
       ...
       */
      const activeTop = toc.scrollTop + (activeRect.top - tocRect.top);

      const lastRealLinkBottom =
        toc.scrollTop + (lastRect.bottom - tocRect.top);

      /*
       * Keep any real padding you added to the bottom of the TOC.
       * The artificial ::after spacer is deliberately excluded.
       */
      const computed = window.getComputedStyle(toc);
      ...
      const centeredTarget =
        activeTop - toc.clientHeight / 2 + activeRect.height / 2;
       ...
       */
      const naturalBottomTarget = Math.max(
        0,
        lastRealLinkBottom + bottomPadding - toc.clientHeight,
      );
       ...
       */
      const target = Math.max(0, Math.min(centeredTarget, naturalBottomTarget));
